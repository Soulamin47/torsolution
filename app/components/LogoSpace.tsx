...
const PARTICLES = Array.from({ length: 34 }, (_, index) => ({
  ...
  top: `${6 + ((index * 53) % 86)}%`,
  size: 2 + (index % 4),
  color: ["#15141a", "#7c5cff", "#00a878", "#ff7a59"][index % 4],
...
export default function LogoSpace() {
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <div className="absolute left-1/2 top-1/2 h-[68%] w-[68%] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#cfc1ff]/48 blur-[54px]" />
      <div className="absolute left-[18%] top-[15%] h-28 w-28 rounded-full bg-[#ffcf4a]/45 blur-[38px]" />
      <div className="absolute bottom-[12%] right-[13%] h-32 w-32 rounded-full bg-[#70e1c1]/40 blur-[42px]" />

      <div className="absolute inset-[6%] [perspective:1100px]">
        <div className="logo-orbit logo-orbit-a absolute inset-[8%] rounded-[50%] border border-[#15141a]/16" />
      ...
      >
        <div className="absolute h-[290px] w-[290px] rounded-[34%] border border-white/70 bg-white/26 shadow-[0_45px_100px_rgba(65,46,135,.16)] backdrop-blur-[2px] [transform:translateZ(-70px)_rotate(8deg)]" />
        <div className="absolute h-[250px] w-[250px] rounded-[34%] border border-[#7c5cff]/16 bg-[#7c5cff]/[0.035] [transform:translateZ(-32px)_rotate(-5deg)]" />
        <motion.div
          className="relative [transform:translateZ(55px)]"
          animate={{ y: [-7, 7, -7], rotate: [-1.2, 1.2, -1.2] }}
          transition={{ duration: 6.5, repeat: Infinity, ease: "easeInOut" }}
        >
          <Image src="/logo-mark-light.svg" alt="" width={260} height={260} priority className="h-[220px] w-[220px] drop-shadow-[0_28px_30px_rgba(32,22,72,.16)] md:h-[280px] md:w-[280px]" />
        </motion.div>
      </motion.div>

      <div className="absolute bottom-[8%] left-1/2 h-px w-[58%] -translate-x-1/2 bg-gradient-to-r from-transparent via-black/16 to-transparent" />
    </div>
  );
}
